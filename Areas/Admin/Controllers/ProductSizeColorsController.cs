...
namespace JDshop.Areas.Admin.Controllers
{
    ...
    public class ProductSizeColorsController : Controller
    {
        ...
        public ProductSizeColorsController(JDshopDbContext context, INotyfService notyfService)
        {
            _context = context;
        ...
        {
            var JDshopDbContext = _context.ProductSizeColors.Include(p => p.Color).Include(p => p.Product).ThenInclude(x => x.Images).Include(p => p.ProductInventory).Include(p => p.Size);
